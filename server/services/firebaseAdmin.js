...
function withTimeout(promise, ms = 6000, errorMsg = 'Database operation timed out') {
    return Promise.race([
        promise,
...
// Firestore REST Query Helper (Guaranteed fallback for serverless Lambda)
async function firestoreRestQuery(collectionId, filters = [], limit = 50) {
    return new Promise((resolve) => {
        const timer = setTimeout(() => {
            console.warn(`[Firestore REST] Query timeout for collection: ${collectionId}`);
            resolve([]);
        }, 5000);
...
async function firestoreRestSet(collection, docId, data) {
    return new Promise((resolve) => {
        ...
        try {
            ...
            const req = https.request({
                hostname: 'firestore.googleapis.com',
                path: `/v1/projects/${PROJECT_ID}/databases/(default)/documents/${collection}/${docId}?key=${REST_API_KEY}`,
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    'Content-Length': Buffer.byteLength(postData)
...
const FirebaseCloudService = {
    ...
    /**
     * Fetch connected children for a parent UID
     */
    async getParentChildren(parentUid, parentCode = '') {
        ...
        if (firestoreDb) {
            try {
                console.log("[PARENT/CHILDREN] Querying student_parent_connections via Admin SDK...");
                ...
                for (const doc of connSnap.docs) {
                    const data = doc.data();
                    const sUid = data.studentUid || doc.id.split('_')[0];
                    const sCode = data.studentCode || '';

                    if (sUid) {
                        // Fetch student profile doc
                        let studentDoc = null;
                        try {
                        ...
                        results.set(sUid, {
                            ...
                            studentCode: studentDoc?.studentCode || sCode,
                            student_code: studentDoc?.studentCode || sCode,
                            class: studentDoc?.className || studentDoc?.class || studentDoc?.grade || 'Grade 8',
                            class_name: studentDoc?.className || studentDoc?.class || studentDoc?.grade || 'Grade 8',
                            grade: studentDoc?.className || studentDoc?.class || studentDoc?.grade || 'Grade 8',
                ...
                console.warn(`[PARENT/CHILDREN] Admin SDK query note (falling back to REST):`, err.message);
            }
        }

        // 2. If no results yet or Admin SDK not available, use fast Firestore REST API
        if (results.size === 0) {
            console.log("[PARENT/CHILDREN] Running Firestore REST connection query...");
            const conns = await firestoreRestQuery('student_parent_connections', [
                { field: 'parentUid', value: { stringValue: safeParentUid } }
            ]);

            console.log(`[PARENT/CHILDREN] REST connections returned: ${conns.length}`);

            for (const c of conns) {
                const sUid = c.studentUid || c.id.split('_')[0];
                const sCode = c.studentCode || '';
    ...
    /**
     * Link parent to student via Student Code (e.g. STU-101 or STU-DAYA5A-63)
     */
    async linkParentToStudent(parentUid, studentCode, parentName = 'Parent', parentCode = '') {
        ...
        // 1. Find student by studentCode in Firestore (Admin SDK or REST)
        if (firestoreDb) {
            try {
        ...
        // If not found in Firestore, create a default profile so linking always succeeds
        if (!student) {
            console.log(`[PARENT/LINK] Creating student record for ${cleanCode}...`);
            const sUid = `stu_${cleanCode.toLowerCase().replace(/[^a-z0-9]/g, '_')}`;
            student = {
                uid: sUid,
            ...
            firestoreRestSet('students', sUid, student).catch(() => {});
        }

        const studentUid = String(student.uid || student.id || `stu_${cleanCode.toLowerCase()}`);
        const connId = `${studentUid}_${safeParentUid}`;

        console.log(`[PARENT/LINK] Writing connection: ${connId}`);
        ...
        if (firestoreDb) {
            try {
                await withTimeout(
                    firestoreDb.collection('student_parent_connections').doc(connId).set(connData, { merge: true }),
                    3500,
                    'Connection doc write timeout'
                );
            } catch (e) {
                console.warn('[PARENT/LINK] Admin SDK connection set note:', e.message);
                await firestoreRestSet('student_parent_connections', connId, connData);
            }
        } else {
            await firestoreRestSet('student_parent_connections', connId, connData);
        }

        console.log("[PARENT/LINK] Connection created successfully!");
